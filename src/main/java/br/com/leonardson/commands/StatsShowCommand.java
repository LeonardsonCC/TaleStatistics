...
public class StatsShowCommand extends AbstractPlayerCommand {
    DefaultArg<String> playerName;

    public StatsShowCommand() {
        super("show", "View player statistics", false);

        this.playerName = this.withDefaultArg("playerName", "The name of the player (empty to self)", ArgTypes.STRING, "self", "Desc of Default: self");
    }
    ...
    protected void execute(CommandContext context, Store<EntityStore> store, Ref<EntityStore> ref, PlayerRef playerRef,
        ...
        try {
            ...
            if (!playerName.equals("self")) {
                rs = Main.getInstance().getDatabaseManager().getPlayerStatsByName(playerName);
            } else {
