...
public class PlayerItemDropEventSystem extends EntityEventSystem<EntityStore, DropItemEvent.Drop> {
    @Nonnull
    private final ComponentType<EntityStore, PlayerRef> playerRefComponentType = PlayerRef.getComponentType();
    private final Main plugin;
    private final DatabaseManager database;

    public PlayerItemDropEventSystem(@Nonnull Main plugin, @Nonnull DatabaseManager database) {
    ...
    public void handle(
        ...
        database.incrementStat(playerRef.getUuid().toString(), "items_dropped", quantity);
        plugin.getLogger().at(Level.FINE).log("Items dropped recorded for " + playerRef.getUsername());
    }
